...
export default function Catalog() {
  ...
  useEffect(() => {
    ...
    (async () => {
      ...
      // Fetch settings (non-blocking: use defaults if anything fails)
      try {
  ...
  // Group products by category, then apply admin category ordering
  const { cats, grouped } = useMemo(() => {
    ...
    // Sort each category's items (in-stock first, then name)
  ...
  // If active category doesn't exist anymore, fall back to Noodles -> first -> All
  useEffect(() => {
  ...
  // Optional: once categories exist, prefer Noodles as default (even if category order changes)
  useEffect(() => {
    if (!cats.length) return;
    if (cats.includes(DEFAULT_CAT)) setActiveCat(DEFAULT_CAT);
  }, [cats.length]);
    ...
    <div className="container-app">
      <div className="flex flex-col gap-6 lg:flex-row">
        <div className="flex-1">
          <div className="mb-4">
            <h1 className="text-2xl font-semibold tracking-tight">Ten Peso Run</h1>
            <p className="mt-1 text-sm text-slate-600">Grab snacks fast. Checkout as guest.</p>
          </div>

          {/* Category filter bar (landing page filter) */}
          {!loading && !error && cats.length > 0 && (
            <div className="sticky top-0 z-10 -mx-1 mb-4 border-b border-slate-200 bg-white/90 px-1 py-2 backdrop-blur">
                ...
                {featuredCategories
                  ...
                  .map((c) => (
                    <button
                      key={c}
                      onClick={() => setActiveCat(c)}
                      className="shrink-0 rounded-full bg-slate-100 px-3 py-1.5 text-sm font-medium text-slate-700 hover:bg-slate-200"
                    >
                      ...
                      return (
                        ...
                        >
                          <div className="relative aspect-square overflow-hidden rounded-xl bg-slate-100">
                            ...
                            {out && (
                              <div className="absolute inset-0 flex items-center justify-center bg-white/60 text-xs font-semibold text-slate-700">
                                Out of stock
                              </div>
                            )}
                          </div>
                          ...
                          <div className="mt-3">
                            ...
                            <div className="mt-1 text-sm text-slate-700">{formatPHP(p.price_cents)}</div>

                            <div className="mt-2 flex items-center justify-between">
                              <span className={`text-xs ${out ? "text-red-600" : "text-slate-500"}`}>
                                {out ? "Out of stock" : `Stock: ${p.stock_qty ?? 0}`}
                              </span>
                            </div>

                            <div className="mt-3 flex items-center justify-between gap-2">
                              <button
                                className="h-9 w-9 rounded-xl border border-slate-200 text-lg disabled:opacity-40"
                                onClick={() => bump(p, -1)}
                                disabled={inCart <= 0}
                                aria-label="remove"
                              >
        ...
        <aside className="lg:w-[340px]">
          <div className="sticky top-20 rounded-2xl border border-slate-200 bg-white p-5 shadow-sm">
            ...
            <div className="mt-4 space-y-2">
              ...
              {cart.slice(0, 6).map((it) => (
                <div key={it.product.id} className="flex items-start justify-between gap-2 text-sm">
                  ...
                  <div className="tabular-nums">{formatPHP(it.product.price_cents * it.qty)}</div>
                </div>
              ))}
            ...
            <div className="mt-4 border-t border-slate-200 pt-4">
              ...
              <Link
                href="/checkout"
                className={`mt-4 inline-flex h-10 w-full items-center justify-center rounded-xl bg-slate-900 text-sm font-semibold text-white ${
